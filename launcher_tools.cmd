...
>>"%MENU_FILE%" echo [03] DEDUPLICATE COLLECTED LICENSES       ^| dedupe              ^| exact content duplicates only
>>"%MENU_FILE%" echo [04] COLLECT + DEDUPLICATE LICENSES       ^| collect_dedupe      ^| safe collect then cleanup
>>"%MENU_FILE%" echo [05] MAKE RELEASE ARCHIVE                 ^| make_release        ^| stage, collect licenses, archive
>>"%MENU_FILE%" echo.
...
if /I "%RAW%"=="collect_dedupe" goto COLLECT_AND_DEDUPE
...
echo [3] Deduplicate collected licenses
echo [4] Collect + Deduplicate licenses
...
if errorlevel 4 goto COLLECT_AND_DEDUPE
if errorlevel 3 goto DEDUPE_LICENSES
...
:COLLECT_AND_DEDUPE
call "%LICENSE_DIR%\Run-Collect-And-Deduplicate-ThirdPartyLicenses.cmd" /CLEAN
goto MAIN
