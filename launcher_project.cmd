...
:FZF_MENU
> "%MENU_FILE%" echo [01] Quick JPG 75                                     ^| quick_jpg_75        ^| convert to JPG 75
>>"%MENU_FILE%" echo [02] Quick PNG balanced                               ^| quick_png_balanced  ^| convert to PNG balanced
>>"%MENU_FILE%" echo [03] Normalize to sRGB, PDF stays PDF                 ^| quick_norm_jpg      ^| choose Pillow or color.org profile
>>"%MENU_FILE%" echo [04] Normalize to CMYK, PDF stays PDF                 ^| quick_norm_cmyk     ^| choose Photoshop5 or FOGRA39
>>"%MENU_FILE%" echo [05] Convert to grayscale (PNG)                       ^| quick_gray_png      ^| batch gray output
>>"%MENU_FILE%" echo [06] Fit with crop to 2160p (JPG)                     ^| quick_2160_jpg      ^| crop to UHD target
>>"%MENU_FILE%" echo [07] Fit with crop to 16:9 (JPG)                      ^| quick_169_jpg       ^| crop to widescreen ratio
>>"%MENU_FILE%" echo [08] TIFF ^> PNG frames 001-999                        ^| split_tiff          ^| multi-page TIFF split
>>"%MENU_FILE%" echo [09] Contact sheet                                    ^| contact_sheet       ^| quick preview grid
>>"%MENU_FILE%" echo [10] Watermark text                                   ^| watermark           ^| batch text overlay
>>"%MENU_FILE%" echo [11] Trim solid border                                ^| trim_border         ^| crop by corner color
>>"%MENU_FILE%" echo [12] DPI without resize                               ^| dpi_only            ^| update metadata only
>>"%MENU_FILE%" echo.
>>"%MENU_FILE%" echo [13] Convert to JPG                                   ^| convert_jpg         ^| quality 60 75 90 + DPI
>>"%MENU_FILE%" echo [14] Convert to PNG                                   ^| convert_png         ^| balanced + DPI
>>"%MENU_FILE%" echo [32] PDF ^> JPG / PNG export                          ^| pdf_export          ^| choose embedded or render
>>"%MENU_FILE%" echo [33] PDF ^> Embedded images (extract)                  ^| pdf_extract         ^| inspect internal raster objects
>>"%MENU_FILE%" echo [34] Images ^> PDF                                     ^| images_to_pdf       ^| lossless preferred or JPG high
>>"%MENU_FILE%" echo [16] Fit to screen 1080p / 1440p / 2160p              ^| resize_screen       ^| crop=trim contain=fit pad=bars
>>"%MENU_FILE%" echo [17] Fit to 16:9 / A4 / A3                            ^| aspect              ^| crop=trim contain=fit pad=bars
>>"%MENU_FILE%" echo [18] Plotter: fit to roll by short or long side       ^| plotter_size        ^| keep pixels, compute DPI from mm
>>"%MENU_FILE%" echo [19] Plotter: DPI without resize                      ^| plotter_dpi_only    ^| keep pixels, write chosen DPI
>>"%MENU_FILE%" echo [20] Split image...                                   ^| split_half          ^| strips or grid ^> PNG
>>"%MENU_FILE%" echo [21] Smart crop white background                      ^| smart_crop_white    ^| trim to first meaningful pixels
>>"%MENU_FILE%" echo [22] Downscale to 25%% or 50%%                          ^| downscale_percent   ^| lanczos or nearest
>>"%MENU_FILE%" echo [35] Photo sheet by width                             ^| photo_sheet_width   ^| no-gap line sheet from input
>>"%MENU_FILE%" echo [36] Photo sheet cut-lines                            ^| photo_sheet_cut     ^| strips by height + cut plan
>>"%MENU_FILE%" echo [23] Formats and adapters                             ^| formats             ^| core optional best-effort
>>"%MENU_FILE%" echo [24] Project information                              ^| info                ^| paths runtime purpose
>>"%MENU_FILE%" echo.
>>"%MENU_FILE%" echo [26] Open INPUT folder                                ^| open_input          ^| explorer input
>>"%MENU_FILE%" echo [27] Open OUTPUT folder                               ^| open_output         ^| explorer output
>>"%MENU_FILE%" echo [25] Environment doctor                               ^| doctor              ^| validate runtime and imports
>>"%MENU_FILE%" echo [28] Open CONFIG folder                               ^| open_config         ^| defaults ICC profiles
>>"%MENU_FILE%" echo [29] Open LOGS folder                                 ^| open_logs           ^| JSON reports
>>"%MENU_FILE%" echo [30] Tools launcher                                   ^| tools               ^| build release service
>>"%MENU_FILE%" echo [31] Russian project launcher                         ^| launcher_ru         ^| switch to russian shell
>>"%MENU_FILE%" echo [00] Exit                                             ^| exit                ^| close

"%FZF_CMD%" --prompt="audion@image-tools [PROJECT-EN] > " --pointer=">" --header="Choose an action. Quick presets first, manual modes and service tools below." --layout=reverse --border=rounded --info=hidden --margin=1,2  < "%MENU_FILE%" > "%RES_FILE%"
...
:FALLBACK_MENU
echo [1] Quick JPG 75
echo [2] Quick PNG balanced
echo [3] Normalize to sRGB, PDF stays PDF
echo [4] Normalize to CMYK, PDF stays PDF
echo [5] Convert to grayscale (PNG)
echo [6] Fit with crop to 2160p (JPG)
echo [7] Fit with crop to 16:9 (JPG)
echo [8] TIFF ^> PNG frames 001-999
echo [9] Contact sheet
echo [A] Watermark text
echo [B] Trim solid border
echo [C] DPI without resize
echo [D] Convert to JPG
echo [E] Convert to PNG
echo [W] PDF ^> JPG / PNG export
echo [X] PDF ^> Embedded images (extract)
echo [Y] Images ^> PDF
echo [G] Fit to screen 1080p / 1440p / 2160p
echo [H] Fit to 16:9 / A4 / A3
echo [I] Plotter: resize for roll by short or long side
echo [J] Plotter: DPI without resize for roll
echo [K] Split image...
echo [L] Smart crop white background
echo [M] Downscale to 25%% or 50%%
echo [Z] Photo sheet modes
echo [Q] Open input folder
echo [R] Open output folder
echo [N] Formats and adapters
echo [O] Project information
echo [P] Environment doctor
echo [S] Open config folder
echo [T] Open logs folder
echo [U] Tools launcher
echo [V] Russian project launcher
echo [0] Exit
echo.
...
if not defined DST_PATH goto MAIN
echo.
set /p TOLERANCE=White background tolerance [10] : 
...
set /p COLS=Columns [4] : 
if not defined COLS set "COLS=4"
set /p THUMB=Thumb size WIDTHxHEIGHT [480x270] : 
...
:PHOTO_SHEET_MENU
echo.
echo [1] Simple no-gap line sheet
echo [2] Cut-lines sheet by matching heights
echo [0] Back
choice /C 120 /N /M "Select photo sheet mode: "
...
:PHOTO_SHEET_WIDTH
echo.
echo Using config: config\photo_sheet.yaml [paper_saver]
...
echo.
echo Using config: config\photo_sheet.yaml [cut_lines]
...
call :ASK_OUTPUT_FORMAT
echo.
set /p WM_TEXT=Watermark text : 
if not defined WM_TEXT goto MAIN
set /p WM_POS=Position bottom-right / bottom-left / top-right / top-left [bottom-right] : 
if not defined WM_POS set "WM_POS=bottom-right"
set /p WM_OPACITY=Opacity 1-255 [160] : 
if not defined WM_OPACITY set "WM_OPACITY=160"
set /p WM_SIZE=Font size [36] : 
...
if defined FZF_CMD (
  ...
  call :RUN_FZF_MENU "PNG preset > " "Choose PNG preset."
...
set "JPEG_QUALITY="
echo.
echo Output format after normalization:
echo   1 - JPG
echo   2 - PNG
echo   3 - JPG with quality 75
...
if defined FZF_CMD (
  ...
  call :RUN_FZF_MENU "sRGB profile > " "Choose sRGB profile."
...
if defined FZF_CMD (
  ...
  call :RUN_FZF_MENU "cmyk profile > " "Choose CMYK profile."
...
if defined FZF_CMD (
  ...
  call :RUN_FZF_MENU "target profile > " "Choose target color space."
...
if defined FZF_CMD (
  ...
  >>"%MENU_FILE%" echo custom ^| custom color #RRGGBB
  call :RUN_FZF_MENU "alpha background > " "Choose alpha background."
  ...
  if /I "%ALPHA_BG%"=="custom" (
    set "ALPHA_BG="
    set /p ALPHA_BG=Enter color #RRGGBB [white] : 
    if not defined ALPHA_BG set "ALPHA_BG=white"
  )
  goto :eof
)
echo.
echo Alpha background:
echo   1 - white
echo   2 - black
echo   3 - custom color #RRGGBB
...
if defined FZF_CMD (
  ...
  call :RUN_FZF_MENU "metadata > " "Choose metadata policy."
...
echo.
echo Mode:
echo   1 - crop      - crop to target
echo   2 - contain   - fit without padding
echo   3 - pad       - fit with padding
echo   4 - by-height - match target height
echo   5 - by-width  - match target width
...
if defined FZF_CMD (
  ...
  call :RUN_FZF_MENU "ratio > " "Choose ratio."
...
if defined FZF_CMD (
  ...
  >>"%MENU_FILE%" echo white ^| white
  >>"%MENU_FILE%" echo custom ^| custom color #RRGGBB
  call :RUN_FZF_MENU "padding color > " "Choose padding color."
  ...
  if /I "%BG_COLOR%"=="custom" (
    set "BG_COLOR="
    set /p BG_COLOR=Enter color #RRGGBB [black] : 
    if not defined BG_COLOR set "BG_COLOR=black"
  )
  goto :eof
)
...
set "TARGET_MM="
echo.
echo Side length in mm:
echo   1 - 600
echo   2 - 900
echo   3 - 1050
set /p TARGET_MM=Enter mm or preset [900] : 
...
set "SPLIT_PARTS="
echo.
set /p SPLIT_PARTS=Parts, 2..10 [2] : 
...
set "SPLIT_COLUMNS="
echo.
set /p SPLIT_ROWS=Rows vertical, 2..10 [2] : 
if not defined SPLIT_ROWS set "SPLIT_ROWS=2"
set /p SPLIT_COLUMNS=Columns horizontal, 2..10 [2] : 
...
echo.
echo PDF DPI:
echo   Enter any number from 1 to 1200, for example 150 200 300 600
:ASK_PDF_DPI_LOOP
set /p PDF_DPI=Select : 
if defined PDF_DPI call :TRIM PDF_DPI
if not defined PDF_DPI (
  echo [INFO] Please enter a DPI number.
  goto ASK_PDF_DPI_LOOP
)
echo(%PDF_DPI%| findstr /r "^[0-9][0-9]*$" >nul
if errorlevel 1 (
  echo [INFO] DPI must be an integer from 1 to 1200.
  goto ASK_PDF_DPI_LOOP
)
if %PDF_DPI% LSS 1 (
  echo [INFO] DPI must be at least 1.
  goto ASK_PDF_DPI_LOOP
)
if %PDF_DPI% GTR 1200 (
  echo [INFO] DPI must be at most 1200.
  goto ASK_PDF_DPI_LOOP
)
goto :eof
...
if not defined PDF_DPI set "PDF_DPI=300"
echo.
echo PDF export mode:
echo   1 - embedded - largest raster on each page
echo   2 - render   - whole page with text and vector ^(%PDF_DPI% DPI^)
echo   3 - render   - choose another DPI
...
echo PDF mode:
echo   1 - Lossless
echo   2 - JPG with quality 75
echo   3 - JPG with quality 90
