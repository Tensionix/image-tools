...
title Audion Image Tools - Project Cleanup
...
for %%I in ("%BASE_DIR%") do set "ROOT_NAME=%%~nxI"
if /I not "%ROOT_NAME%"=="Audion Image Tools" (
  echo [ERROR] Unexpected project folder:
  echo %BASE_DIR%
  echo.
  echo This cleanup script is allowed to run only from an "Audion Image Tools" project root.
  call :WAIT_KEY
  exit /b 1
)

if not exist "%BASE_DIR%\system_core\main.py" (
  echo [ERROR] Project marker not found:
  echo %BASE_DIR%\system_core\main.py
  echo.
...
if not exist "%BASE_DIR%\system_core\image_tools\commands.py" (
  echo [ERROR] Project marker not found:
  echo %BASE_DIR%\system_core\image_tools\commands.py
  echo.
  echo Cleanup aborted.
  call :WAIT_KEY
  exit /b 1
...
echo ======================================================================
echo   AUDION IMAGE TOOLS - PROJECT CLEANUP
echo ======================================================================
echo Root:
echo   %BASE_DIR%
echo.
echo This script keeps source scripts, folders, documentation and configs.
...
echo.
echo Protected: system_core source, config files including ui_colors.yaml and ICC profiles, docs, launchers, install scripts, license notices and records.
echo.
...
rem licenses\licenses.zip is intentionally preserved by release cleanup.
...
echo [OK] Project cleanup finished.
goto cleanup_done
